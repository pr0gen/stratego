...
pub fn play_many_turns(
    ...
    for i in 0..turn {
        play_one_game(file_name.clone(), first_ai_name.clone() , second_ai_name.clone(), i)
    }
}
...
fn play_one_game(
    ...
    let board =
        engine_utils::create_stratego_board_from_file("./boards.txt", String::from("B")).unwrap();
    ...
    loop {
        ...
        match engine_utils::game_is_over(board.state()) {
            Some(Color::Red) => {
                println!("==========\n{}", board.display());
                println!("Red wins");
                let (red, blue) = evaluation_function::material_evaluation(board, &material_values);
